...
let
  ...
  rufloWorkflowBlock = pkgs.writeText "ruflo-workflow-block.md" ''
    ...
    1. Call `mcp__ruflo__memory_search` with your task description to find prior patterns
    2. Call `mcp__ruflo__hooks_route` with `{ task: "<your task description>" }` for domain routing
    3. Review matches — if confidence > 0.7, apply the learned pattern (roles, approach, strategy)

    ### After Completing Work
    Store your results in agentDB:
    1. `mcp__ruflo__agentdb_hierarchical-store` with:
       - `category`: your agent role (e.g., "coder", "researcher", "tester")
       - `level`: "task-output"
       - `key`: descriptive key for this task
       - `data`: your results summary
