...
### 3. Agent Registration Audit

Verify that every `.md` file in `claude-agents/` has a corresponding `home.file` entry in `claude-agents.nix`.

```bash
# List agent files in repo
find ~/dotfiles/claude-agents -name "*.md" -type f | sort

# List registered agents in nix module
grep 'home.file' ~/dotfiles/nix/modules/claude-agents.nix
```

**What to check:**
- Every `.md` file under `claude-agents/` has a `home.file` entry in `claude-agents.nix`
- No `home.file` entries reference agent files that don't exist
...
| Agent not available in Claude | Missing `home.file` entry in `claude-agents.nix` | Add the registration entry |
