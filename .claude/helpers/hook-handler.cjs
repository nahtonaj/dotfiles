...
const handlers = {
  'route': () => {
    ...
    if (router && router.routeTask) {
      ...
      console.log('');
      console.log(`[TASK_ROUTING]`);
      console.log(`[RECOMMENDED_AGENT: ${result.agent}]`);
      console.log(`[CONFIDENCE: ${(result.confidence * 100).toFixed(0)}%]`);
      console.log(`[COMPLEXITY: ${complexity}]`);
      if (complexity !== 'LOW') {
        console.log(`[AUTO_SPAWN_AGENT: ${result.agent}]`);
      }
    } else {
  ...
  'daemon-init': () => {
    ...
    try {
      execFileSync('bash', ['.claude/helpers/daemon-manager.sh', 'start', '30', '60'], { timeout: 3000, stdio: 'pipe' });
      console.log('[OK] Daemon startup initiated');
    } catch (e) {
      console.log('[WARN] Daemon startup skipped: ' + e.message);
    }
  },
