...
patch_once() {
  ...
  if [[ $((clear_count + clear_patched_count)) != 1 \
    ...
    || $((capture_count + capture_patched_count)) != 1 ]]; then
    printf 'claude-mem summary patch refused for %s: clear=%s/%s routing=%s/%s resume=%s/%s capture=%s/%s\n' \
      "$worker" \
      "$clear_count" "$clear_patched_count" \
      "$routing_count" "$routing_patched_count" \
      "$resume_count" "$resume_patched_count" \
      "$capture_count" "$capture_patched_count" >&2
    return 1
  fi
  ...
  python3 - "$worker" \
    ...
    "$anchor_capture" "$replacement_capture" > "$tmp_file" <<'PY'
import sys
